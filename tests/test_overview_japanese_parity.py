...
JA_PAGE = ARTICLE_DIR / "ja" / "index.html"
JA_OVERVIEW_SVG = (
    REPO_ROOT / "docs" / "assets" / "when-reasoning-pays-off-overview.ja.svg"
)
...
def _local_reference(html: str, attribute: str, filename: str) -> Path:
    ...
    return (JA_PAGE.parent / unquote(parsed.path)).resolve()
...
def test_japanese_overview_matches_korean_baseline_blocks() -> None:
    ko_html = KO_PAGE.read_text(encoding="utf-8")
    ja_html = JA_PAGE.read_text(encoding="utf-8")
    shared_markers = (
    ...
    for marker in shared_markers:
        ...
        assert marker in ja_html

    assert _main_child_signature(ja_html) == _main_child_signature(ko_html)
    assert ja_html.count("<figure") == ko_html.count("<figure")
    assert 'aria-label="このシリーズの構成"' in ja_html
    assert "when-reasoning-pays-off-overview.ja.svg" in ja_html


def test_japanese_overview_local_references_resolve() -> None:
    html = JA_PAGE.read_text(encoding="utf-8")
    references = (
        ("href", "when-reasoning-pays-off-overview.ja.svg"),
        ("src", "when-reasoning-pays-off-overview.ja.svg"),
...
def test_japanese_overview_svg_is_present_and_localized() -> None:
    svg = JA_OVERVIEW_SVG.read_text(encoding="utf-8")
    assert "<title" in svg
    assert "推論" in svg
    assert "トークン" in svg
    assert "請求" in svg
...
def test_japanese_chart_locale_matches_korean_keyset_without_fallback() -> None:
    ko = json.loads((LOCALE_DIR / "ko.json").read_text(encoding="utf-8"))
    ja = json.loads((LOCALE_DIR / "ja.json").read_text(encoding="utf-8"))
    assert _leaf_paths(ja) == _leaf_paths(ko)
    assert ja["meta"]["fallback"] is False
    assert ja["meta"]["translation_status"] == "translated"
